...
# Corregir los nombres no detectados (Lógica Fuzzy)
def corregir_nombre_equipo(nombre_api, lista_db):
    if not lista_db:
        return nombre_api
    
    # 1. Intentar match directo (limpiando espacios)
    nombre_api = nombre_api.strip()
    if nombre_api in lista_db:
        return nombre_api

    # 2. Fuzzy Match con un umbral más bajo (50) para nombres traducidos
    # Usamos token_set_ratio que es mejor para nombres largos o con variaciones
    ...
    # DEBUG: Para que veas en la consola de VS Code qué está pasando
    # print(f"API: {nombre_api} -> DB: {mejor_match} (Score: {score})")
...
# CARGAMOS LISTA DE EQUIPOS REALES DE LA DB PARA EL MATCHING
try:
...
try:
    ...
    # --- APLICACIÓN DEL FILTRO INTELIGENTE ---
...
with col1:
    ...
    if not df_h2h.empty:
        ...
        st.info(f"Sin enfrentamientos previos registrados para {home_team} vs {away_team}.")

    st.subheader("Tendencia de Goles (Últimos 10 partidos)")
    q_trend = f"""
        SELECT FTHG, FTAG FROM historial_multiliga_ml 
...
with col2:
    ...
    if model:
        ...
        st.write(f"**Goles esperados (media):** {promedio_goles:.2f}")
        st.progress(prob_over, text=f"Probabilidad Over 2.5: {prob_over:.1%}")

        st.markdown("#### **Proyecciones de Juego**")
        t1, t2 = st.tabs(["Tiros a Puerta", "Córners"])
        with t1:
            st.write(f"**{home_team}:** {stats_h['HST']:.1f} esperados")
            st.write(f"**{away_team}:** {stats_a['AST']:.1f} esperados")
        with t2:
            st.write(f"**{home_team}:** {stats_h['HC']:.1f} córners")
            st.write(f"**{away_team}:** {stats_a['AC']:.1f} córners")

# --- SECCIÓN DE DISCIPLINA (Tarjetas) ---
st.divider()
st.subheader("Análisis de Disciplina y Tarjetas")
col_disc1, col_disc2 = st.columns(2)

with col_disc1:
    st.markdown("#### **Promedio Amarillas (Últimos 5 partidos)**")
    c1, c2 = st.columns(2)
    c1.metric(f"Media {home_team}", f"{stats_h['HY']:.1f}")
    c2.metric(f"Media {away_team}", f"{stats_a['AY']:.1f}")
    
    if stats_h['HY'] > 2.5 or stats_a['AY'] > 2.5:
        st.warning("⚠️ ¡Atención! Tendencia alta de tarjetas.")
